...
function statusOf(a: any, urgent: boolean) {
  ...
  return { label: '입찰 대기', bar: 'bg-slate-300', text: 'text-slate-500' };
}
      ...
      <div className="max-w-7xl mx-auto rounded-3xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-3 px-5 sm:px-6 py-3.5 border-b border-slate-100">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-rose-50 text-rose-600 text-[12.5px] font-extrabold tracking-wide shrink-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[270px_1fr]">
          ...
          <div className="p-5 sm:p-6 lg:border-r border-slate-100 bg-slate-50/60">
            <div className="flex items-center gap-4">
              ...
              <div className="relative w-[104px] h-[104px] shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-[12.5px] text-slate-500 leading-none mb-0.5">진행 중</span>
            ...
            {top && (
              <div className="mt-3 rounded-xl bg-amber-50 border border-amber-100 px-3 py-2.5">
                <div className="flex items-center gap-1.5 text-[12px] font-bold text-amber-700 mb-0.5">
          ...
          <div className="p-3 sm:p-4">
            <div className="hidden md:grid grid-cols-[36px_1.5fr_1.6fr_0.9fr_0.9fr] gap-3 px-3 pb-2 text-[12.5px] font-bold text-slate-500 tracking-wide">
              ...
              {auctions.map((a, i) => {
                ...
                return (
                  <li key={a.id}>
                    ...
                    >
                      {/* 순위 */}
                      <span className={`w-6 h-6 rounded-lg inline-flex items-center justify-center text-[12px] font-extrabold tabular-nums ${RANK_STYLE[rank - 1] || 'bg-slate-100 text-slate-500'}`}>
                        {rank}
                      </span>
                      ...
                      {/* 모바일: 남은 시간 (첫 줄 오른쪽) */}
                      <span className={`md:hidden text-[12px] font-bold tabular-nums text-right whitespace-nowrap ${t.urgent ? 'text-rose-600' : 'text-slate-500'}`}>
                        {t.text}
                      </span>
                        ...
                        {a.brandCount === 0 ? (
                          ...
                          <>
                            {a.brandInitials.map((c: string, i: number) => (
                              <span key={i} className="w-6 h-6 rounded-full bg-slate-100 border border-white text-[12.5px] font-bold text-slate-600 inline-flex items-center justify-center -ml-1.5 first:ml-0">
                                {c}
                              </span>
                            ))}
                            {a.brandCount > a.brandInitials.length && (
                              <span className="text-[12.5px] font-bold text-slate-500 ml-0.5">+{a.brandCount - a.brandInitials.length}</span>
                            )}
                          </>
                        )}
                      </div>
        ...
        <div className="flex items-center justify-between gap-3 px-5 sm:px-6 py-2.5 border-t border-slate-100 bg-slate-50/60">
          <div className="min-w-0 text-[12px] text-slate-500 truncate">
          ...
          <span className="shrink-0 inline-flex items-center gap-1.5 text-[12px] text-slate-500">
...
function SumRow({ label, value, danger }: { label: string; value: number; danger?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-2">
      <dt className="text-[12px] text-slate-500 break-keep">{label}</dt>
  ...
  return (
    <div className="rounded-xl bg-white border border-slate-200 px-3 py-2.5">
      <div className="text-lg font-extrabold text-slate-900 tabular-nums leading-none">
        ...
        <span className="text-[12px] font-semibold text-slate-500 ml-0.5">{unit}</span>
      </div>
