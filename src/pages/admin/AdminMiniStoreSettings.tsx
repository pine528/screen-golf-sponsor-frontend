            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5">
              ...
              <div className="space-y-2 mb-4">
                {(store.products || []).map((p: any, idx: number) => (
                  <div key={p.id} className="flex items-center gap-2 p-2 border border-slate-100 rounded-lg">
                    <div className="flex flex-col">
                      <button onClick={() => moveProduct(store.products, idx, 'up')} disabled={idx === 0} className="p-0.5 text-slate-500 hover:text-slate-700 disabled:opacity-20"><ArrowUp className="w-3 h-3" /></button>
                      <button onClick={() => moveProduct(store.products, idx, 'down')} disabled={idx === (store.products.length - 1)} className="p-0.5 text-slate-500 hover:text-slate-700 disabled:opacity-20"><ArrowDown className="w-3 h-3" /></button>
                    </div>
                    ...
                    {p.soldOut ? (
                      <span className="text-[12.5px] font-bold text-rose-600 bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-full">품절</span>
                    ) : p.stock <= 5 ? (
                      <span className="text-[12.5px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">재고 {p.stock}</span>
                    ) : (
                      <span className="text-xs text-slate-500">재고 {p.stock}</span>
                    )}
                ...
                {(!store.products || store.products.length === 0) && (
                  <div className="text-center py-6 text-xs text-slate-500 bg-slate-50 rounded">아직 상품이 없습니다</div>
                )}
              </div>
