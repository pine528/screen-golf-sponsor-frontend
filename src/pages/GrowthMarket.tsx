            ...
            <div className="flex flex-wrap gap-2">
              {BENEFITS.map((b) => (
                <div key={b.title} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
                  <b.icon className="w-4 h-4 text-slate-500 shrink-0" />
                  <div className="leading-tight">
                    <div className="text-[12px] font-bold text-slate-900 whitespace-nowrap">{b.title}</div>
                    <div className="text-[12.5px] text-slate-500 whitespace-nowrap">{b.desc}</div>
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={h.title} className="flex items-center gap-4 flex-1">
                  <div className="flex items-start gap-3 flex-1">
                    <div className="relative shrink-0">
                      ...
                      <span className="absolute -top-1.5 -left-1.5 w-5 h-5 rounded-full bg-emerald-600 text-white text-[12.5px] font-black flex items-center justify-center tabular-nums">
                        {i + 1}
                      </span>
                    </div>
                    <div className="min-w-0">
...
export function StoreCard({ store, products, wide = false }: { store: FanStore; products: any[]; wide?: boolean }) {
  ...
  const cta = (
    <span
      className={`mt-auto inline-flex items-center justify-center gap-1 h-9 rounded-lg text-[13px] font-bold ${
        to ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500'
      }`}
    >
      {to ? '스토어 보기' : '오픈 준비 중'}
  ...
  const chips = (
    <div className="flex flex-wrap items-center gap-1.5 mb-3">
      {store.benefit && (
        <span className="px-2 py-1 rounded-md bg-emerald-50 text-emerald-700 text-[12px] font-bold">{store.benefit}</span>
      )}
      <span className="px-2 py-1 rounded-md border border-slate-200 text-slate-500 text-[12px] font-bold">AR 보기</span>
    </div>
  );
    ...
    <div className="relative aspect-square rounded-lg border border-slate-100 bg-slate-50 overflow-hidden">
      {product.imageUrl ? (
        ...
        <div className="w-full h-full flex items-center justify-center p-1">
          <span className="text-[8px] leading-tight text-slate-500 text-center line-clamp-3 break-keep">{product.name}</span>
        </div>
      )}
      {off > 0 && (
...
function StripProduct({ product }: { product: any }) {
  ...
  return (
    <div className="flex items-center gap-2.5 px-3.5 py-2.5 min-w-0">
      ...
      <div className="min-w-0">
        <div className="text-[12px] text-slate-600 truncate">{product.name}</div>
      ...
      <div className="relative aspect-square bg-slate-50">
        {product.imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center p-3">
            <span className="text-[12px] text-slate-500 text-center break-keep leading-snug line-clamp-4">{product.name}</span>
          </div>
        )}
        {off > 0 && (
          <span className="absolute top-2 left-2 px-1.5 py-0.5 rounded-md bg-rose-500 text-white text-[12.5px] font-black">{off}%</span>
        )}
      </div>
      <div className="p-2.5">
        <div className="text-[12px] text-slate-600 line-clamp-2 min-h-[2rem] mb-1">{product.name}</div>
        <div className="flex items-baseline gap-1.5">
          {list > price && <span className="text-[12.5px] text-slate-500 line-through tabular-nums">{list.toLocaleString()}원</span>}
          <span className="text-sm font-black text-slate-900 tabular-nums">{price.toLocaleString()}원</span>
        </div>
        {purchasable && (
          <div className="mt-1.5 inline-flex items-center justify-center gap-1 w-full h-7 rounded-lg border border-emerald-200 text-emerald-700 text-[12px] font-bold">
