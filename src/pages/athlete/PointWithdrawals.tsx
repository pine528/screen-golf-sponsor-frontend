...
export default function AthletePointWithdrawals() {
  ...
  const { data: balanceData, isLoading: loadingBalance } = useQuery({
    queryKey: ['pointWithdrawalBalance'],
    queryFn: () => api.getPointWithdrawalBalance(),
  });

  // 내 출금 요청 목록
  const { data: withdrawalsData, isLoading: loadingWithdrawals } = useQuery({
    queryKey: ['myPointWithdrawals', statusFilter],
    queryFn: () => api.getMyPointWithdrawals({ status: statusFilter || undefined }),
  });

  // 출금 요청 생성
  const createMutation = useMutation({
    mutationFn: (data: typeof form) =>
      api.createPointWithdrawal({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['pointWithdrawalBalance'] });
      queryClient.invalidateQueries({ queryKey: ['myPointWithdrawals'] });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (balance && amount > balance.available) {
      alert(`출금 가능 금액(${balance.available.toLocaleString()}P)을 초과했습니다`);
      return;
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white rounded-lg border p-4">
            ...
            <p className="text-2xl font-bold text-gray-900">
              {loadingBalance ? '...' : `${(balance?.balance || 0).toLocaleString()}P`}
            </p>
          </div>
          <div className="bg-white rounded-lg border p-4">
            <p className="text-sm text-gray-500">출금 가능</p>
            <p className="text-2xl font-bold text-emerald-600">
              {loadingBalance ? '...' : `${(balance?.available || 0).toLocaleString()}P`}
            </p>
          </div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-blue-700">
              <p className="font-medium mb-1">포인트 출금 안내</p>
              <ul className="list-disc list-inside space-y-0.5">
                <li>최소 출금 금액: 1,000P</li>
                <li>출금 요청 후 관리자 승인이 필요합니다</li>
                <li>승인 후 영업일 기준 1~3일 내 지급됩니다</li>
                <li>1포인트 = 1원으로 환산됩니다</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border p-4">
          {!showForm ? (
            <button
              ...
              className="w-full py-3 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <h3 className="font-semibold text-lg">포인트 출금 요청</h3>
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  출금 가능: {(balance?.available || 0).toLocaleString()}P (= {(balance?.available || 0).toLocaleString()}원)
                </p>
              </div>
