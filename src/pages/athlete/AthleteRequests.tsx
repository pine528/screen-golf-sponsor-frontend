...
import {
  AlertTriangle, Check, CheckCircle2, Clock, Inbox, Monitor, Pencil, Shirt, Wallet, XCircle,
} from 'lucide-react';
...
export default function AthleteRequests() {
  ...
  const load = useCallback(async () => {
    try {
      const [r, d]: any[] = await Promise.all([
        api.getAthleteRequests(),
        api.getAthleteDigitalRequests().catch(() => ({ data: [] })),
      ]);
      setItems(r?.data || []);
      setDigital(d?.data || []);
    } catch (e: any) {
  ...
  const actDigital = async (app: any, action: 'APPROVE' | 'REVISION' | 'REJECT') => {
    const key = `dg:${app.id}`;
    if (action !== 'APPROVE' && open?.itemId !== key) { setOpen({ itemId: key, action }); setComment(''); return; }
    setBusy(key);
    setErr(null);
    try {
      await api.reviewDigitalApplication(app.id, {
        action,
        ...(action !== 'APPROVE' ? { reasonCode: reason, comment: comment || REASONS.find((r) => r.code === reason)?.label } : {}),
      });
      setOpen(null);
  ...
  const digitalPending = digital.filter((d) => d.status === 'SUBMITTED');
    ...
    <div className="min-h-screen bg-slate-50/60 text-slate-900 pb-16">
      ...
      <section className="max-w-4xl mx-auto px-5 pt-8">
        ...
        <p className="mt-2 text-[13.5px] text-slate-500">
          ...
          {pending.length + digitalPending.length > 0 && (
            <span className="ml-1 font-bold text-emerald-600">승인 대기 {pending.length + digitalPending.length}건</span>
          )}
        </p>
        ...
        {digital.length > 0 && (
          <div className="mt-10">
            ...
            <div className="mt-4 space-y-3">
              {digital.map((d) => {
                const key = `dg:${d.id}`;
                const due = d.approvalDueAt ? new Date(d.approvalDueAt) : null;
                const expired = due ? due.getTime() < Date.now() : false;
                const label = d.status === 'SUBMITTED' ? '승인 대기'
                  : d.status === 'ATHLETE_APPROVED' ? '승인 완료'
                  : d.status === 'ACTIVE' ? '구독 중'
                  : d.status === 'NEEDS_REVISION' ? '수정 요청함' : '거절함';
                const cls = d.status === 'SUBMITTED' ? 'bg-amber-50 text-amber-700'
                  : d.status === 'ATHLETE_APPROVED' || d.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700'
                  : d.status === 'NEEDS_REVISION' ? 'bg-sky-50 text-sky-700' : 'bg-slate-100 text-slate-500';
                return (
                      ...
                      open?.itemId === key ? (
                        <div className="mt-4 rounded-xl bg-slate-50 border border-slate-200 p-4">
                          ...
                          <textarea
                            ...
                            onChange={(e) => setComment(e.target.value.slice(0, 300))}
                            rows={2}
                            placeholder="브랜드에 전달할 내용을 적어주세요"
                            className="w-full rounded-lg border border-slate-200 px-3.5 py-2.5 text-[13px] focus:outline-none focus:border-emerald-400 resize-none"
                          />
                          <div className="mt-3 flex gap-2 justify-end">
                            <button onClick={() => setOpen(null)} className="h-10 px-4 rounded-xl border border-slate-200 text-[13px] font-bold text-slate-600">취소</button>
                            <button
                              onClick={() => actDigital(d, open.action)}
                              disabled={busy === key}
                              className={`h-10 px-5 rounded-xl text-white text-[13px] font-bold disabled:opacity-50 ${
                                open.action === 'REJECT' ? 'bg-slate-700 hover:bg-slate-800' : 'bg-sky-600 hover:bg-sky-700'
                              }`}
                            >
                              {busy === key ? '처리 중…' : open.action === 'REJECT' ? '거절하기' : '수정 요청 보내기'}
                            </button>
                          </div>
                        </div>
                      ) : (
                        <div className="mt-4 flex flex-wrap gap-2 pt-4 border-t border-slate-100">
                          <button
                            onClick={() => actDigital(d, 'APPROVE')}
                            disabled={busy === key}
                            className="h-11 px-6 inline-flex items-center gap-1.5 rounded-xl bg-emerald-600 text-white text-[13.5px] font-bold hover:bg-emerald-700 disabled:opacity-50"
                          >
                            <Check className="w-4 h-4" /> {busy === key ? '처리 중…' : '승인하기'}
                          </button>
                          <button
                            onClick={() => { setOpen({ itemId: key, action: 'REVISION' }); setComment(''); }}
                            className="h-11 px-5 inline-flex items-center gap-1.5 rounded-xl border border-sky-300 text-sky-700 text-[13.5px] font-bold hover:bg-sky-50"
                          >
                            <Pencil className="w-4 h-4" /> 조건 조정 요청
                          </button>
                          <button
                            onClick={() => { setOpen({ itemId: key, action: 'REJECT' }); setComment(''); }}
                            className="h-11 px-5 inline-flex items-center gap-1.5 rounded-xl border border-slate-200 text-slate-600 text-[13.5px] font-bold hover:bg-slate-50"
                          >
                            <XCircle className="w-4 h-4" /> 거절
                          </button>
